...
export function AttractScreen({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden text-center">
      ...
      {/* sits above the tap target so choosing a language doesn't start a game */}
      ...
      <button
        type="button"
        onClick={onStart}
        aria-label={t.attract.tapToPlay}
      ...
      <div className="pointer-events-none relative z-20 flex flex-col items-center gap-[4vmin] px-[6vmin]">
        ...
        <p className="max-w-[100vmin] text-[2.6vmin] font-medium leading-snug text-white/60">
          {t.attract.subtitleA}
          <br />
          <span className="text-white">{t.attract.subtitleB}</span>
        </p>
      </div>

      {/* ticker */}
      <div className="pointer-events-none absolute bottom-0 left-0 right-0 z-20 overflow-hidden border-t border-edge bg-black/70 py-[1.6vmin]">
        <div className="flex w-max animate-drift whitespace-nowrap">
          {[0, 1].map((copy) => (
            <div key={copy} className="flex">
              {t.attract.ticker.map((item) => (
                <span
                  key={`${copy}-${item}`}
